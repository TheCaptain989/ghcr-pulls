...
while IFS= read -r line; do
    ...
    if [ -n "$tag" ]; then
      pages=$(curl -sSLN https://github.com/"$owner"/"$repo"/pkgs/container/"$image"/versions | grep -Pzo '(?<=<em class="current" data-total-pages=")\d*')
      [ -z "$pages" ] && pages=1
      for i in $(seq 1 "$pages"); do
         curl -sSLN https://github.com/"$owner"/"$repo"/pkgs/container/"$image"/versions?page="$i" >page.html
         pulls=$(xmllint --html --recover --xpath "//a[text()=\"$tag\"]/../../../div[2]/span/text()" page.html 2>/dev/null | tr -d '\f\n ')
         raw_pulls=$(echo "$pulls" | tr -d ',')
         [ -n "$pulls" ] && break
      done
      rm page.html
    else
...
for i in $(jq -r '.[] | @base64' index.json); do
    ...
    # add new badge
    s/\n\n(\[!\[.*)\n\n/\n\n$1 \[!\[$owner\/$repo\/$image\/$tag\]\(https:\/\/img.shields.io\/badge\/dynamic\/json\?url=https%3A%2F%2Fraw.githubusercontent.com%2Fthecaptain989%2Fghcr-pulls%2Fmaster%2Findex.json\&query=%24%5B%3F(%40.owner%3D%3D%22$owner%22%20%26%26%20%40.repo%3D%3D%22$repo%22%20%26%26%20%40.image%3D%3D%22$image%22%22%20%26%26%20%40.tag%3D%3D%22$tag%22)%5D.pulls\&label=$image\/$tag\)\]\(https:\/\/github.com\/$owner\/$repo\/pkgs\/container\/$image\)\n\n/g;
